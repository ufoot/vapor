...
func TestX32Cos(t *testing.T) {
	...
	for f = -20.0; f < 20.0; f += 0.5 {
		x1 = X32Cos(vpnumber.F64ToX32(f))
		x2 = vpnumber.F64ToX32(math.Cos(f))
		if vpnumber.X32IsSimilar(x1, x2) {
			t.Logf("similar cos values for f=%f x1=%d x2=%d", f, x1, x2)
		} else {
			t.Errorf("inconsistent cos values for f=%f x1=%d x2=%d", f, x1, x2)
		}
	}
}

func TestX64Cos(t *testing.T) {
	var f float64
	var x1 vpnumber.X64
	var x2 vpnumber.X64

	for f = -20.0; f < 20.0; f += 0.5 {
		x1 = X64Cos(vpnumber.F64ToX64(f))
		x2 = vpnumber.F64ToX64(math.Cos(f))
