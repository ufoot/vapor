...
func TestBruijnGenericNext(t *testing.T) {
	...
	for i, v := range bruijn23 {
		...
		nf, err := implGeneric.NextFirst(bigToBytes(bi, implGeneric.NbBytes()))
		if err != nil {
		...
		nl, err := implGeneric.NextLast(bigToBytes(bi, implGeneric.NbBytes()))
		if err != nil {
		...
		nList, err := implGeneric.NextList(bigToBytes(bi, implGeneric.NbBytes()))
		if err != nil {
...
func TestBruijnGenericPrev(t *testing.T) {
	...
	for i, v := range bruijn23 {
		bi := big.NewInt(int64(i))
		pf, err := implGeneric.PrevFirst(bigToBytes(bi, implGeneric.NbBytes()))
		if err != nil {
		...
		pl, err := implGeneric.PrevLast(bigToBytes(bi, implGeneric.NbBytes()))
		if err != nil {
		...
		pList, err := implGeneric.PrevList(bigToBytes(bi, implGeneric.NbBytes()))
		if err != nil {
...
func TestBruijnGenericForwardPath(t *testing.T) {
	...
	path, err := implGeneric.ForwardPath(bigToBytes(big.NewInt(from), implGeneric.NbBytes()), bigToBytes(big.NewInt(to), implGeneric.NbBytes()))
	if err != nil {
	...
	for i, v := range path {
		...
		if i > 0 {
			found := false
			nList, err := implGeneric.NextList(path[i-1])
			if err != nil {
				t.Error("unable to call NextList:", err)
			}
			for j, w := range nList {
		...
		w, err := implGeneric.ForwardElem(bigToBytes(big.NewInt(from), implGeneric.NbBytes()), bigToBytes(big.NewInt(to), implGeneric.NbBytes()), i)
		if err != nil {
...
func TestBruijnGenericBackwardPath(t *testing.T) {
	...
	path, err := implGeneric.BackwardPath(bigToBytes(big.NewInt(from), implGeneric.NbBytes()), bigToBytes(big.NewInt(to), implGeneric.NbBytes()))
	if err != nil {
	...
	for i, v := range path {
		...
		if i > 0 {
			found := false
			nList, err := implGeneric.PrevList(path[i-1])
			if err != nil {
				t.Error("unable to call PrevList:", err)
			}
			for j, w := range nList {
		...
		w, err := implGeneric.BackwardElem(bigToBytes(big.NewInt(from), implGeneric.NbBytes()), bigToBytes(big.NewInt(to), implGeneric.NbBytes()), i)
		if err != nil {
