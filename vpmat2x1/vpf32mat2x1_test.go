...
func TestF32Mat2x1Math(t *testing.T) {
	...
	m1 = F32Mat2x1New(f11, f12, f21, f22)
	if !m1.IsSimilar(m1) {
	...
	m2 = F32Mat2x1New(f51, f52, f61, f62)
	m3 = F32Mat2x1Add(m1, m2)
	m4 = F32Mat2x1New(f11+f51, f12+f52, f21+f61, f22+f62)
	if !m3.IsSimilar(m4) {
		t.Error("Add error")
	}
	...
	m4 = F32Mat2x1New(f11*fmul, f12*fmul, f21*fmul, f22*fmul)
	if !m3.IsSimilar(m4) {
...
func BenchmarkF32Mat2x1Add(b *testing.B) {
	mat := F32Mat2x1New(vpnumber.F32Const1, vpnumber.F32Const1, vpnumber.F32Const1, vpnumber.F32Const1)
