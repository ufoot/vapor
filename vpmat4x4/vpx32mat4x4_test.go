...
func TestX32Mat4x4x4Math(t *testing.T) {
	...
	var m1, m2, m3, m4 *X32Mat4x4x4

	m1 = X32Mat4x4x4New(x11, x12, x13, x14, x21, x22, x23, x24, x31, x32, x33, x34, x41, x42, x43, x44)
	if !m1.IsSimilar(m1) {
	...
	m2 = X32Mat4x4x4New(x51, x52, x53, x54, x61, x62, x63, x64, x71, x72, x73, x74, x81, x82, x83, x84)
	m3 = X32Mat4x4x4Add(m1, m2)
	m4 = X32Mat4x4x4New(x11+x51, x12+x52, x13+x53, x14+x54, x21+x61, x22+x62, x23+x63, x24+x64, x31+x71, x32+x72, x33+x73, x34+x74, x41+x81, x42+x82, x43+x83, x44+x84)
	if !m3.IsSimilar(m4) {
		t.Error("Add error")
	}

	m3 = X32Mat4x4x4Sub(m1, m2)
	m4 = X32Mat4x4x4New(x11-x51, x12-x52, x13-x53, x14-x54, x21-x61, x22-x62, x23-x63, x24-x64, x31-x71, x32-x72, x33-x73, x34-x74, x41-x81, x42-x82, x43-x83, x44-x84)
	if !m3.IsSimilar(m4) {
		t.Error("Sub error")
	}

	m3 = X32Mat4x4x4MulScale(m1, xmul)
	m4 = X32Mat4x4x4New(vpnumber.X32Mul(x11, xmul), vpnumber.X32Mul(x12, xmul), vpnumber.X32Mul(x13, xmul), vpnumber.X32Mul(x14, xmul), vpnumber.X32Mul(x21, xmul), vpnumber.X32Mul(x22, xmul), vpnumber.X32Mul(x23, xmul), vpnumber.X32Mul(x24, xmul), vpnumber.X32Mul(x31, xmul), vpnumber.X32Mul(x32, xmul), vpnumber.X32Mul(x33, xmul), vpnumber.X32Mul(x34, xmul), vpnumber.X32Mul(x41, xmul), vpnumber.X32Mul(x42, xmul), vpnumber.X32Mul(x43, xmul), vpnumber.X32Mul(x44, xmul))
	if !m3.IsSimilar(m4) {
		t.Error("MulScale error")
	}

	m3 = X32Mat4x4x4DivScale(m3, xmul)
	if !m3.IsSimilar(m1) {
...
func invertableX32Mat4x4x4() *X32Mat4x4x4 {
	var ret X32Mat4x4x4
...
func TestX32Mat4x4x4Comp(t *testing.T) {
	m1 := invertableX32Mat4x4x4()
	m2 := X32Mat4x4x4Inv(m1)
	id := X32Mat4x4x4Identity()
...
func TestX32Mat4x4x4Aff(t *testing.T) {
	...
	mr := X32Mat4x4x4RotX(vpmath.X32ConstPi2)
	...
	mr = X32Mat4x4x4RotY(vpmath.X32ConstPi2)
	t.Logf("rotation mat4x4 for PI/2 is %s", mr.String())
	v2 = mr.MulVec(v1)
	t.Logf("mat4x4 MulVec %s * %s = %s", mr.String(), v1.String(), v2.String())
	...
	mr = X32Mat4x4x4RotZ(vpmath.X32ConstPi2)
	t.Logf("rotation mat4x4 for PI/2 is %s", mr.String())
	v2 = mr.MulVec(v1)
	t.Logf("mat4x4 MulVec %s * %s = %s", mr.String(), v1.String(), v2.String())
...
func TestX32Mat4x4x4JSON(t *testing.T) {
	m1 := invertableX32Mat4x4x4()
	m2 := X32Mat4x4x4Identity()
	...
	if err == nil {
		t.Logf("encoded JSON for X32Mat4x4x4 is \"%s\"", string(jsonBuf))
	} else {
		t.Error("unable to encode JSON for X32Mat4x4x4")
	}
	err = m2.UnmarshalJSON([]byte("nawak"))
	if err == nil {
		t.Error("able to decode JSON for X32Mat4x4x4, but json is not correct")
	}
	err = m2.UnmarshalJSON(jsonBuf)
	if err != nil {
		t.Error("unable to decode JSON for X32Mat4x4x4")
	}
	if !m1.IsSimilar(m2) {
...
func BenchmarkX32Mat4x4x4Add(b *testing.B) {
	mat := X32Mat4x4x4New(vpnumber.X32Const1, vpnumber.X32Const1, vpnumber.X32Const1, vpnumber.X32Const1, vpnumber.X32Const1, vpnumber.X32Const1, vpnumber.X32Const1, vpnumber.X32Const1, vpnumber.X32Const1, vpnumber.X32Const1, vpnumber.X32Const1, vpnumber.X32Const1, vpnumber.X32Const1, vpnumber.X32Const1, vpnumber.X32Const1, vpnumber.X32Const1)
...
func BenchmarkX32Mat4x4x4Inv(b *testing.B) {
	mat := invertableX32Mat4x4x4()

	for i := 0; i < b.N; i++ {
		_ = X32Mat4x4x4Inv(mat)
	}
}
