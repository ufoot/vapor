...
func TestX64Mat3x3Math(t *testing.T) {
	...
	var m1, m2, m3, m4 *X64Mat3x3

	m1 = X64Mat3x3New(x11, x12, x13, x21, x22, x23, x31, x32, x33)
	if !m1.IsSimilar(m1) {
	...
	m2 = X64Mat3x3New(x51, x52, x53, x61, x62, x63, x71, x72, x73)
	m3 = X64Mat3x3Add(m1, m2)
	m4 = X64Mat3x3New(x11+x51, x12+x52, x13+x53, x21+x61, x22+x62, x23+x63, x31+x71, x32+x72, x33+x73)
	if !m3.IsSimilar(m4) {
		t.Error("Add error")
	}

	m3 = X64Mat3x3Sub(m1, m2)
	m4 = X64Mat3x3New(x11-x51, x12-x52, x13-x53, x21-x61, x22-x62, x23-x63, x31-x71, x32-x72, x33-x73)
	if !m3.IsSimilar(m4) {
		t.Error("Sub error")
	}

	m3 = X64Mat3x3MulScale(m1, xmul)
	m4 = X64Mat3x3New(vpnumber.X64Mul(x11, xmul), vpnumber.X64Mul(x12, xmul), vpnumber.X64Mul(x13, xmul), vpnumber.X64Mul(x21, xmul), vpnumber.X64Mul(x22, xmul), vpnumber.X64Mul(x23, xmul), vpnumber.X64Mul(x31, xmul), vpnumber.X64Mul(x32, xmul), vpnumber.X64Mul(x33, xmul))
	if !m3.IsSimilar(m4) {
		t.Error("MulScale error")
	}

	m3 = X64Mat3x3DivScale(m3, xmul)
	if !m3.IsSimilar(m1) {
...
func invertableX64Mat3x3() *X64Mat3x3 {
	var ret X64Mat3x3
...
func TestX64Mat3x3Comp(t *testing.T) {
	m1 := invertableX64Mat3x3()
	m2 := X64Mat3x3Inv(m1)
	id := X64Mat3x3Identity()
...
func TestX64Mat3x3JSON(t *testing.T) {
	m1 := invertableX64Mat3x3()
	m2 := X64Mat3x3Identity()
	...
	if err == nil {
		t.Logf("encoded JSON for X64Mat3x3 is \"%s\"", string(jsonBuf))
	} else {
		t.Error("unable to encode JSON for X64Mat3x3")
	}
	err = m2.UnmarshalJSON([]byte("nawak"))
	if err == nil {
		t.Error("able to decode JSON for X64Mat3x3, but json is not correct")
	}
	err = m2.UnmarshalJSON(jsonBuf)
	if err != nil {
		t.Error("unable to decode JSON for X64Mat3x3")
	}
	if !m1.IsSimilar(m2) {
...
func BenchmarkX64Mat3x3Add(b *testing.B) {
	mat := X64Mat3x3New(vpnumber.X64Const1, vpnumber.X64Const1, vpnumber.X64Const1, vpnumber.X64Const1, vpnumber.X64Const1, vpnumber.X64Const1, vpnumber.X64Const1, vpnumber.X64Const1, vpnumber.X64Const1)
...
func BenchmarkX64Mat3x3Inv(b *testing.B) {
	mat := invertableX64Mat3x3()
