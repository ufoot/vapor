...
// It modifies the vector, and returns a pointer on it.
func (vec *F64Vec3) Dot(op *F64Vec3) *F64Vec3 {
	for i, v := range op {
		vec[i] *= v
	}
...
// F64Vec3SqMag returns the sum of the squares of all values.
// It is used to calculate length, it is faster than the complete
// length calculation, as it does not perform a square root.
func F64Vec3SqMag(vec *F64Vec3) float64 {
	return vec.SqMag()
}
...
// F64Vec3IsSimilar returns true if vectors are approximatively the same.
// This is a workarround to ignore rounding errors.
func F64Vec3IsSimilar(veca, vecb *F64Vec3) bool {
	return veca.IsSimilar(vecb)
}

// F64Vec3Dot returns the dot products of two vectors.
// Args are left untouched, a pointer on a new object is returned.
func F64Vec3Dot(veca, vecb *F64Vec3) *F64Vec3 {
	var ret = *veca
