...
func TestF64Mat3x3x3Math(t *testing.T) {
	const f11 = 3.0
	const f12 = 333.0
	const f13 = 31.0
	const f21 = -4.0
	...
	var m1, m2, m3, m4 *F64Mat3x3x3

	m1 = F64Mat3x3x3New(f11, f12, f13, f21, f22, f23, f31, f32, f33)
	if !m1.IsSimilar(m1) {
	...
	m2 = F64Mat3x3x3New(f51, f52, f53, f61, f62, f63, f71, f72, f73)
	m3 = F64Mat3x3x3Add(m1, m2)
	m4 = F64Mat3x3x3New(f11+f51, f12+f52, f13+f53, f21+f61, f22+f62, f23+f63, f31+f71, f32+f72, f33+f73)
	if !m3.IsSimilar(m4) {
		t.Error("Add error")
	}

	m3 = F64Mat3x3x3Sub(m1, m2)
	m4 = F64Mat3x3x3New(f11-f51, f12-f52, f13-f53, f21-f61, f22-f62, f23-f63, f31-f71, f32-f72, f33-f73)
	if !m3.IsSimilar(m4) {
		t.Error("Sub error")
	}

	m3 = F64Mat3x3x3MulScale(m1, fmul)
	m4 = F64Mat3x3x3New(f11*fmul, f12*fmul, f13*fmul, f21*fmul, f22*fmul, f23*fmul, f31*fmul, f32*fmul, f33*fmul)
	if !m3.IsSimilar(m4) {
		t.Error("MulScale error")
	}

	m3 = F64Mat3x3x3DivScale(m3, fmul)
	if !m3.IsSimilar(m1) {
...
func invertableF64Mat3x3x3() *F64Mat3x3x3 {
	var ret F64Mat3x3x3
...
func TestF64Mat3x3x3Comp(t *testing.T) {
	m1 := invertableF64Mat3x3x3()
	m2 := F64Mat3x3x3Inv(m1)
	id := F64Mat3x3x3Identity()
...
func TestF64Mat3x3x3Aff(t *testing.T) {
	const p1 = 3.0
	const p2 = 4.0
	const t1 = 6.5
	const t2 = 8.5
...
func TestF64Mat3x3x3JSON(t *testing.T) {
	m1 := invertableF64Mat3x3x3()
	m2 := F64Mat3x3x3Identity()
	...
	if err == nil {
		t.Logf("encoded JSON for F64Mat3x3x3 is \"%s\"", string(jsonBuf))
	} else {
		t.Error("unable to encode JSON for F64Mat3x3x3")
	}
	err = m2.UnmarshalJSON([]byte("nawak"))
	if err == nil {
		t.Error("able to decode JSON for F64Mat3x3x3, but json is not correct")
	}
	err = m2.UnmarshalJSON(jsonBuf)
	if err != nil {
		t.Error("unable to decode JSON for F64Mat3x3x3")
	}
	if !m1.IsSimilar(m2) {
...
func BenchmarkF64Mat3x3x3Add(b *testing.B) {
	mat := F64Mat3x3x3New(vpnumber.F64Const1, vpnumber.F64Const1, vpnumber.F64Const1, vpnumber.F64Const1, vpnumber.F64Const1, vpnumber.F64Const1, vpnumber.F64Const1, vpnumber.F64Const1, vpnumber.F64Const1)
...
func BenchmarkF64Mat3x3x3Inv(b *testing.B) {
	mat := invertableF64Mat3x3x3()

	for i := 0; i < b.N; i++ {
		_ = F64Mat3x3x3Inv(mat)
	}
}
