...
// F32Cubic1d returns the cubic Bezier curve from p0 to p3,
...
func F32Cubic1d(p0, p1, p2, p3 float32, beta float32) (float32, float32) {
	switch {
	case vpnumber.F32IsSimilar(p0, p1):
		return F32Quadratic1d(p0, p2, p3, beta)
	case vpnumber.F32IsSimilar(p2, p3):
		return F32Quadratic1d(p0, p1, p3, beta)
...
// F32Cubic2d returns the cubic Bezier curve from p0 to p3,
// going through p1 and p2. Points p1 and p2 are typically not reached,
// but they influence the curve, as it heads to p1 from p0, and conversely
// from p3 to p2. Works on float32 2d vectors.
// First returned value is the position, second is the derivative.
func F32Cubic2d(p0, p1, p2, p3 *vpvec2.F32, beta float32) (*vpvec2.F32, *vpvec2.F32) {
	switch {
	case p0.IsSimilar(p1):
		return F32Quadratic2d(p0, p2, p3, beta)
	case p2.IsSimilar(p3):
		return F32Quadratic2d(p0, p1, p3, beta)
	case beta < vpnumber.F32Const0:
...
// F32Cubic3d returns the cubic Bezier curve from p0 to p3,
// going through p1 and p2. Points p1 and p2 are typically not reached,
// but they influence the curve, as it heads to p1 from p0, and conversely
// from p3 to p2. Works on float32 3d vectors.
// First returned value is the position, second is the derivative.
func F32Cubic3d(p0, p1, p2, p3 *vpvec3.F32, beta float32) (*vpvec3.F32, *vpvec3.F32) {
	switch {
	case p0.IsSimilar(p1):
		return F32Quadratic3d(p0, p2, p3, beta)
	case p2.IsSimilar(p3):
		return F32Quadratic3d(p0, p1, p3, beta)
	case beta < vpnumber.F32Const0:
